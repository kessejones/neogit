...
function M.rebase_editor(target, on_close)
  local content = uv_utils.read_file_sync(target)
  RebaseEditorBuffer.new(content, target, on_close):open()
end

function M.commit_editor(target, on_close)
  local content = uv_utils.read_file_sync(target)
  CommitEditorBuffer.new(content, target, on_close):open()
end
